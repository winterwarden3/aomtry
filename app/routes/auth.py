...
def get_recent_login_attempts(username, ip_address):
    """Get number of failed login attempts in last LOCKOUT_MINUTES"""
    try:
        ...
        if response.count and response.count >= MAX_LOGIN_ATTEMPTS:
            return True, response.count
        return False, response.count or 0
...
def get_recent_otp_count(username):
    ...
    try:
        cutoff = (datetime.now() - timedelta(hours=OTP_WINDOW_HOURS)).isoformat()
...
def login():
    ...
    if request.method == 'POST':
        ...
        if is_locked:
            flash(f'Too many failed login attempts. Please wait {LOCKOUT_MINUTES} minutes before trying again.', 'danger')
    ...
    if request.method == 'POST':
        ...
        if action == 'send_otp':
            ...
            if recent_count >= MAX_OTP_REQUESTS:
                flash(f'Too many OTP requests. Please wait {OTP_WINDOW_HOURS} hour before trying again.', 'danger')
                return redirect(url_for('auth.forgot_password', type=forgot_type))
            ...
            remaining = MAX_OTP_REQUESTS - (recent_count + 1)
            ...
            try:
                ...
                if stored_data.get('otp') != otp:
                    ...
                    remaining_attempts = 5 - (stored_data.get('attempts', 0) + 1)
                    flash(f'Wrong OTP. {remaining_attempts} attempts remaining.', 'danger')
                    return render_template('auth/forgot_password.html', username=username, user_type=forgot_type)
                
            except Exception as e:
                print(f"Error verifying OTP: {e}")
                flash('Error verifying OTP. Please try again.', 'danger')
                return redirect(url_for('auth.forgot_password', type=forgot_type))
            ...
            if result:
                # Clean up
                supabase.table("otp_requests").delete().eq("username", username).execute()
